...
FROM employees e   WHERE e.department_id = t.department_id   ),0) 
AS 평균연봉 FROM  ( SELECT     d.*, loc.street_address, loc.postal_code 
FROM departments d LEFT JOIN locations loc ON d.location_id = loc.location_id ) t;

...
BEGIN
    ...
    -- :OLD는 테이블 DELETE, UPDATE가 적용되기 전 기존 데이터이며 변경 전 데이터를 뜻함
...
END;
-- tbl_user가 UPDATE와 DELETE가 진행한 후 TRIGGER 진행하는데 trg_user_backup 테이블에 결과를 넣어줌
...
-- 아래를 실행했을 때 tbl_user에서는 변경이 되었지만,
-- tbl_user_backup에서는 변경 전에 있던 결과와 수정이 업데이트 된 것을 볼 수 있다.
-- 그 이유는 tbl_user_backup에 :OLD를 넣었기 때문이다.
UPDATE tbl_user SET address = '인천' WHERE id = 'test01';
-- 아래도 마찬가지로 변경 전에 있던 결과와 삭제가 업데이트 된 것을 볼 수 있다.
...
(product_seq.NEXTVAL,'피자',100,10000);
INSERT INTO product VALUES
(product_seq.NEXTVAL,'치킨',100,20000);
INSERT INTO product VALUES
(product_seq.NEXTVAL,'햄버거',10,5000);
...
(order_history_seq.NEXTVAL, 200, 1, 5, 50000);
INSERT INTO order_history VALUES
(order_history_seq.NEXTVAL, 200, 2, 1, 20000);
INSERT INTO order_history VALUES
(order_history_seq.NEXTVAL, 200, 3, 5, 25000);

SELECT * FROM order_history;
-- 수량이 빠진 이유는 INSERT 할때 수량값을 넣었기 때문에 그 값만큼 빠진 것이다.
SELECT * FROM product;

INSERT INTO order_history VALUES
(order_history_seq.NEXTVAL, 201, 3, 5, 30000);

INSERT INTO order_history VALUES
(order_history_seq.NEXTVAL, 202, 3, 1, 5000);

-- 트리거 내에서 예외가 발생하면 수행 중인 INSERT 작업은 중단되며 ROLLBACK이 진행됩니다.
INSERT INTO order_history VALUES
(order_history_seq.NEXTVAL, 202, 1, 100, 100000);
